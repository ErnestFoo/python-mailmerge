...
class TextMailMerge(BaseMailMerge):
    ...
    def __is_zone_array(self, zone: dict) -> bool:
        ...
        return "item_count" in zone.zone_keys and isinstance(
            zone.zone_keys["item_count"], int
        )
    ...
    def __list_replace(self, zone: dict):
        ...
        """
        start_tag, end_tag = self.__get_start_end_tags(zone.zone_name)
        template_regions = self.__get_string_region(
            self._temp_buffer, start_tag, end_tag
        )
...
if __name__ == "__main__":
    ...
    # mailmerge.save_output_from_buffer("output.md")
