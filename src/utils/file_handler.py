...
class FileHandler:
    ...
    def __init__(self, mode=FileHandlerMode.READ, file_path: str = None):
        ...
        self.__file_type: validInputFileTypes = (
            None  # Metadata on the extension/type of file
        )

        if mode == FileHandlerMode.READ and file_path:
            self.load_file(file_path)

        if (
            mode == FileHandlerMode.WRITE or mode == FileHandlerMode.APPEND
        ) and file_path:
    ...
    def _write_mode_validation(self):
        if self.mode is FileHandlerMode.WRITE or self.mode is FileHandlerMode.APPEND:
            return True
        else:
            raise ValueError(
                f"FileHandler is not in write/append mode, current mode: {self.mode}"
            )

    def _read_mode_validation(self):
        if self.mode is FileHandlerMode.READ:
            return True
        else:
            raise ValueError(
                f"FileHandler is not in read mode, current mode: {self.mode}"
            )
    ...
    def __write_dir_check__(self, directory: str) -> bool:
        ...
        if not os.path.exists(directory):
            ...
            print(f"Directory does not exist created directory at {directory}")
...
if __name__ == "__main__":
    ...
    writer = FileHandler(
        mode=FileHandlerMode.WRITE, file_path="Sample_Input/output_path.md"
    )
